...
const { app, BrowserWindow, ipcMain } = electron; // ES6 nová syntaxe
let window; //ES6 nový syntaxe, pouze informuje interpret že tato proměnná se bude používat
let homeDir;
...
function startApp() {
  ...
  fs.mkdir(path.join(os.homedir(), "demo-app"), () => {
    homeDir = path.join(os.homedir(), "demo-app");
  });

  window = new BrowserWindow({}); // Instancovaní jednoho okna (vlákna)
  window.loadURL(
    `file://${path.resolve(__dirname, "..", "view", "index.html")}`
  ); // Načtení HTML do okna ze složky view
}
